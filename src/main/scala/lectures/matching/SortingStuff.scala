...
object SortingStuff extends App {
  ...
  //  // Метод должен положить вещь в правильную коробку
    private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox) = item match {
      case watch@Watches(_, _) if watch.cost > 1000 => stuffBox.copy(watches = watch :: stuffBox.watches)
      case boots@Boots(_, _) if boots.brand == "Adidas" || boots.brand == "Converse" =>  stuffBox.copy(boots = boots :: stuffBox.boots)
      case book@Book(_, _) if book.isInteresting => stuffBox.copy(books = book :: stuffBox.books)
      case junk@_ => stuffBox.copy(junk = junk :: stuffBox.junk)
    }

    def findMyKnife(stuffBox: StuffBox): Boolean = stuffBox match {
      case stuffBox@StuffBox(_,_,_, junk) if junk.contains(Knife) => true
      case _ => false
    }

  // //вместо вопросов подставьте композицию функций  sortJunk и findMyKnife
   val knifeIsInJunk = (sortJunk _ andThen findMyKnife _)(stuff)
